<h1 align="center">Hey, I'm Atlazuka</h1>
<h3 align="center">Front-End Developer · Web & App Development</h3>

---

### About
I build interactive web and app experiences and enjoy working close to the logic behind them.

Right now, I’m developing a **real-time anti-cheat system for multiplayer games**, focused on detecting abnormal client behavior on the server side. I like projects that involve **live data, pattern analysis, and performance-sensitive systems**.

I’m currently expanding into backend development with **NestJS and Spring Boot**, aiming to build more complete, scalable applications.

---

### What I’m Working With
- **Languages:** JavaScript, Python  
- **Frontend:** React, HTML, CSS  
- **Backend / Runtime:** Node.js  
- **Frameworks:** NestJS, Spring Boot  
- **Tools:** Git, REST APIs, real-time event handling  

---

### Open To
- Web applications
- Game-related tools & services
- Developer-focused utilities

---

### Contact
- Email: **atlazuka1@gmail.com**
- Discord:  
  <a href="https://discord.com/users/1281741256518795317">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/discord.svg" height="28" width="38" />
  </a>
- Twitter:  
  <a href="https://twitter.com/atlazuka18">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/twitter.svg" height="28" width="38" />
  </a>

---

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=atlazuka&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=atlazuka&theme=dark" />
</p>
